...
last_true_rot = Quaternion([1, 0, 0.2, 0.1]).normalised
true_rot = Quaternion([1, 0.1, 0.25, -0.05]).normalised
...
opti.subject_to(norm_2(w + 0.00001) < 0.5)
...
k = np.array([0.1, 0.25, -0.05, 1])
print(rotate_q(derivative.as_rotvec(), k/np.linalg.norm(k)))
print(last_true_rot)
try:
    result = opti.solve()
    q = result.value(quat)
    ...
    last_quaternion = Quaternion(scalar = last_q[3], vector = last_q[:3]).normalised
    print("FEASIBLE")
